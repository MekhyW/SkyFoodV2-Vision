...
with dai.Pipeline(device) as pipeline:
    ...
    nn_archive = dai.NNArchive(dai.getModelFromZoo(model_description, apiKey=api_key))
    ...
    snaps_producer = pipeline.create(SnapsProducer).build(nn_with_parser.passthrough, nn_with_parser.out, label_map=nn_archive.getConfigV1().model.heads[0].metadata.classes)
